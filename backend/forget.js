...
async function handleSendOTP() {
  ...
  try {
    const res = await fetch(`${API_BASE}?sheet=professor&action=send_otp&email=${encodeURIComponent(email)}`);
    ...
    if (data.status === 'success') {
      ...
      showError('step1-error', data.message || 'Failed to send OTP.');
    }
  } catch (err) {
    showError('step1-error', 'Connection error. Please try again.');
  } finally {
    btn.innerHTML = '<i class="fa-solid fa-paper-plane"></i> Send Recovery OTP';
    btn.disabled = false;
  }
}
...
async function handleVerifyOTP() {
  ...
  try {
    const res = await fetch(`${API_BASE}?sheet=professor&action=verify_otp_only&email=${encodeURIComponent(verifiedEmail)}&otp=${encodeURIComponent(otp)}`);
    const data = await res.json();
...
async function handleResetPassword() {
  ...
  try {
    const res = await fetch(`${API_BASE}?sheet=professor&action=verify_otp&email=${encodeURIComponent(verifiedEmail)}&otp=${encodeURIComponent(otp)}&newPassword=${encodeURIComponent(newPass)}`);
    const data = await res.json();
